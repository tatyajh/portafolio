...
export default function GameCard({ game, index }: { game: Game; index: number }) {
  return (
    ...
    >
      ...
      {/* Sello game dev */}
      <div className="absolute -top-9 -right-3 sm:-right-9 stamp w-24 h-24 px-2 text-[9px] uppercase tracking-[0.15em] font-serif text-burgundy bg-paper-deep">
        Game · Dev · {game.year}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10 text-center">
        {[
          ...
          <div key={item.label} className="stitch-border-gold p-3 bg-paper-deep/50">
            ...
            <p className="font-serif text-ink">{item.value}</p>
          </div>
        ))}
      </div>
          ...
          <div className="stitch-border-gold p-5 bg-paper-deep/30 divide-y divide-brown/25">
            {game.architecture.map(item => (
              <div key={item.label} className="py-3 first:pt-0 last:pb-0 grid sm:grid-cols-[9rem_1fr] gap-1 sm:gap-4">
                <p className="text-[11px] uppercase tracking-widest text-brown font-serif">{item.label}</p>
                <p className="text-ink/85 text-sm sm:text-base leading-relaxed">{item.detail}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-4">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="px-8 py-4 bg-burgundy text-[#f7f1e4] font-serif tracking-wider hover:bg-burgundy-dark transition-all flex items-center gap-3"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="px-8 py-4 stitch-border text-ink font-serif tracking-wider hover:bg-burgundy/5 transition-all flex items-center gap-3"
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
