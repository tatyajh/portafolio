# Mi Vaquita

## Orden de imágenes

1. `assets/mivaquita-1.png` — acceso.
2. `assets/mivaquita-2.png` — inicio.
3. `assets/mivaquita-3.png` — calendario.
4. `assets/mivaquita-4.png` — amigos.
5. `assets/mivaquita-5.png` — grupos.

Usa primero `assets/00-cover-pintada.png` como portada ilustrada y después las cinco capturas reales.

## Texto para Behance

**Título:** Mi Vaquita — app para organizar gastos compartidos

**Descripción:** Mi Vaquita reúne grupos, gastos, fechas y aportes en un mismo lugar. Diseñé la interfaz para que organizar un paseo y saber quién debe qué fuera sencillo.

**Mi rol:** Diseño de interfaz y desarrollo frontend y backend.

**Tecnologías:** React · Express · Node.js.

**Enlace:** https://mi-vaquita-fe.vercel.app
